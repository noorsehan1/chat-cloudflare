...
class RoomManager {
    ...
    if (data) {
      if (data.seats) {
        for (const [seat, seatData] of Object.entries(data.seats)) {
          this.seats.set(parseInt(seat), seatData);
        }
      }
      if (data.points) {
  ...
  toJSON() {
    ...
    return { seats, points, muted: this.muted, number: this.number };
  }
...
export class ChatHandler {
  ...
  async _loadStateFromD1() {
    try {
      // Load room states
      const roomsData = await this.env.DB.prepare(
        "SELECT room_name, data FROM chat_rooms"
      ).all();
      
      if (roomsData.results && roomsData.results.length > 0) {
        for (const row of roomsData.results) {
          if (row.data) {
            try {
              const data = JSON.parse(row.data);
              const roomManager = new RoomManager(row.room_name, data);
              this.rooms.set(row.room_name, roomManager);
            } catch(e) {}
          }
        }
      }
      ...
      // Load current number
      const numResult = await this.env.DB.prepare(
        "SELECT value FROM chat_config WHERE key = 'current_number'"
      ).first();
      
      if (numResult) {
        this.currentNumber = parseInt(numResult.value) || 1;
      }
  ...
  async _saveStateToD1() {
    try {
      for (const [roomName, roomMan] of this.rooms) {
        if (roomMan) {
          const data = JSON.stringify(roomMan.toJSON());
          await this.env.DB.prepare(
            `INSERT INTO chat_rooms (room_name, data, updated_at) 
             VALUES (?, ?, CURRENT_TIMESTAMP)
             ON CONFLICT(room_name) DO UPDATE SET data = ?, updated_at = CURRENT_TIMESTAMP`
          ).bind(roomName, data, data).run();
        }
      }
      
      await this.env.DB.prepare(
        `INSERT INTO chat_config (key, value, updated_at) 
         VALUES ('current_number', ?, CURRENT_TIMESTAMP)
         ON CONFLICT(key) DO UPDATE SET value = ?, updated_at = CURRENT_TIMESTAMP`
      ).bind(String(this.currentNumber), String(this.currentNumber)).run();
