...
const C = {
  ...
  ALARM_INTERVAL_MS: 900000,    // 15 MENIT (TETAP)
  NUMBER_UPDATE_TIK: 6,         // 6 × 15 menit = 90 menit
  MAX_NUMBER: 6,
  BATCH_SIZE: 20,
  LOCK_TIMEOUT: 10000,
  CLEANUP_INTERVAL: 600000,     // 10 MENIT (TETAP)
  MAX_EVENT_QUEUE: 100,
...
export class ChatServer {
  ...
  _startIdleChecker() {
    this._idleCheckInterval = setInterval(() => {
      ...
      const hasUsers = this.wsSet.size > 0;
      
      if (idleTime > this._idleTimeout && !hasUsers && !this._isIdle) {
        this._isIdle = true;
        this._pauseIntervals();
      } else if ((hasUsers || idleTime < this._idleTimeout) && this._isIdle) {
        this._isIdle = false;
  ...
  async alarm() {
    ...
    // ✅ SKIP ALARM JIKA IDLE ATAU TIDAK ADA USER
    if (this._isIdle || this.wsSet.size === 0) {
      try {
