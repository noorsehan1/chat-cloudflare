...
// VERSION: 14.3.0 - DISABLED HIBERNATION & REMOVED RESTORE
// ============================================================
...
// ============================================================
// ALARM SCHEDULER - MODIFIED (NO RESTORE FROM SLEEP)
// ============================================================
...
class AlarmScheduler {
  constructor(db, ctx) {
    ...
    this._hibernationDisabled = true;
  }
  ...
  async scheduleAlarms() {
    try {
      ...
      if (currentSession) {
        ...
        if (endDelay > 0) {
          this._scheduleAlarmInternal('dice_session_end', endDelay);
        }
        this._scheduleAlarmInternal('dice_session_start_immediate', 1000);
        return true;
      }
      ...
      if (nextSession) {
        ...
        this._scheduleAlarmInternal('dice_session_start', startDelay);
        const endTotal = parseTime(nextSession.end);
        const endDelay = (endTotal - currentTotal) * 60 * 1000;
        if (endDelay > 0) this._scheduleAlarmInternal('dice_session_end', endDelay);
      }
      return true;
  ...
  async _scheduleWeeklyResetUTC() {
    try {
      ...
      if (delayMs > 0) this._scheduleAlarmInternal(CONSTANTS.WEEKLY_RESET_ALARM, delayMs);
      return true;
    } catch(e) { return false; }
  }

  _scheduleAlarmInternal(name, delayMs) {
    try {
...
export class GameServer {
  constructor(state, env) {
    try {
      ...
      // === HIBERNATION DISABLED ===
      this._hibernationEnabled = false;
      this._restoreOnWake = false;
      ...
      // Initialize server (load from DB but no restore from sleep)
  ...
  // ============================================================
  // INITIALIZE SERVER - LOAD FROM DB, NO RESTORE FROM SLEEP
  // ============================================================
  ...
  async _initializeServer() {
    try {
      ...
      // Initialize alarm scheduler (without restoring from storage)
      ...
      for (const [username, conn] of this.userConnections) {
        if (conn.room === room && conn.wsId) {
          ...
          if (ws && ws.readyState === 1 && !ws._closing) {
            ...
            this.wsClients.get(room).add(conn.wsId);
            this.clientRooms.set(conn.wsId, room);
            ws.room = room;
            ws.roomname = room;
          }
        }
  ...
  async webSocketMessage(ws, message) {
    ...
    if (!this._initialized || this._isRestoring) {
    ...
    try {
      ...
      if (evt === "getDiceNotification") {
        try {
          const isDiceTime = this.alarmScheduler.isDiceTime();
          const isActive = this.currentDiceRoll && this._canSubmitDiceAnswer;
          const timeLeft = this.alarmScheduler._getTimeLeftUntilNextDice();
          let notification = "";
          if (isActive) {
  ...
  // ============================================================
  // GAME: CLOSE REGISTRATION (LOWCARD)
  // ============================================================
