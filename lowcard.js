...
// Versi stabil: reconnect aman, kursi tidak hilang, LowCard lengkap
...
export class ChatServer {
  ...
  handleMessage(ws, raw) {
    ...
    switch (evt) {
      case "setIdTarget": {
        ...
        if (this.pendingCleanup.has(newId)) {
          clearTimeout(this.pendingCleanup.get(newId));
          this.pendingCleanup.delete(newId);
        }
        ...
        if (this.userToSeat.has(newId)) {
          const { room, seat } = this.userToSeat.get(newId);
          const seatMap = this.roomSeats.get(room);
          const info = seatMap.get(seat);
  ...
  cleanupClient(ws) {
    ...
    if (id && room) {
        if (this.pendingCleanup.has(id)) clearTimeout(this.pendingCleanup.get(id));
        ...
        this.pendingCleanup.set(id, timeout);
    } else {
        ws.numkursi?.clear?.();
        this.clients.delete(ws);
        ws.roomname = undefined;
