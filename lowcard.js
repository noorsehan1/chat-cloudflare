...
export class LowCardGameManager {
    ...
    startGame(ws, betAmount) {
        ...
        this.activeGame = {
            ...
            betAmount: bet,
            countdownTimers: [],
            registrationTime: 30,  // join 30s
            drawTime: 20           // draw 20s
        };

        // Broadcast start message profesional & gaul
        this.chatServer.broadcastToRoom(ws.roomname, [
            "gameLowCardStart",
            `⚡ LowCard is live! Join in 30s and show your skills. Bet: ${bet}`
        ]);
    ...
    joinGame(ws) {
        ...
        // Broadcast join message gaul
        this.chatServer.broadcastToRoom(this.activeGame.room, [
            "gameLowCardJoin",
            ws.idtarget,
            `${ws.idtarget} just hopped in! Welcome aboard! 🎮`
        ]);
    }
    ...
    closeRegistration() {
        ...
        this.chatServer.broadcastToRoom(this.activeGame.room, [
            ...
            "All players locked in! Type .id to draw your cards. Good luck! 🃏"
        ]);
