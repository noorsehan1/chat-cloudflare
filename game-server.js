// ==================== GAME-SERVER.JS ====================
// VERSION: 3.0.4 - FULL COMPLETE - CACHE FIRST, AUTO RESET ON WEEK CHANGE
...
class DiceGameSystem {
  ...
  async loadScores() {
    try {
      ...
      for (const [username, score] of Object.entries(fixedPoints)) {
  ...
  // ============ GET LAST WEEK WINNER - AUTO CHECK & RESET ============
  async getLastWeekWinner() {
    try {
      if (!this.env?.QUESTIONS) return null;
      
      const now = new Date();
      const currentWeek = this.generateCurrentWeek(now);
      
      // 1. AMBIL LAST RESET WEEK DARI CACHE
      let lastResetWeek = this.gameServer._cachedResetWeek;
      if (!lastResetWeek) {
        lastResetWeek = await this.env.QUESTIONS.get(CONSTANTS.DICE_LAST_RESET_WEEK);
        if (lastResetWeek) {
          this.gameServer._cachedResetWeek = lastResetWeek;
          this.gameServer._cachedResetWeekTimestamp = Date.now();
        }
      }
      
      // 2. CEK APAKAH MINGGU BERUBAH
      const weekChanged = lastResetWeek && lastResetWeek !== currentWeek;
      
      // 3. JIKA MINGGU BERUBAH → RESET OTOMATIS
      if (weekChanged) {
        // AMBIL POINTS SEBELUM RESET
        ...
        this.gameServer._cachedResetWeek = currentWeek;
        this.gameServer._cachedResetWeekTimestamp = Date.now();
        
        // KEMBALIKAN WINNER MINGGU LALU
        return winnerData;
      }
      
      // 4. MINGGU SAMA → AMBIL WINNER DARI CACHE
...
export class GameServer extends CPUProtection {
  constructor(state, env) {
    try {
      ...
      // ========== HANYA 1 INTERVAL UNTUK KESEHATAN ==========
      ...
      // ========== INIT SAAT DEPLOY - AMBIL SEMUA DATA DARI KV ==========
      this._initOnDeploy();
  ...
  async _initOnDeploy() {
    try {
      ...
      if (this.env?.QUESTIONS) {
        ...
        this.diceGameSystem._isLoaded = true;
        this.diceGameSystem._lastLoadTime = Date.now();

        // 2. AMBIL LAST WEEK WINNER
        const winnerData = await this.env.QUESTIONS.get(CONSTANTS.DICE_LAST_WEEK_WINNER, 'json');
        if (winnerData) {
        ...
        // 5. AMBIL SEMUA RECORDING STATUS UNTUK SEMUA ROOM
        try {
          ...
          for (const key of list.keys) {
            ...
            if (roomName) {
              ...
              this._recordingEnabled.set(roomName, value === 'true');
            }
          }
        } catch(e) {}

        // 6. AMBIL SEMUA WINNERS UNTUK SEMUA ROOM (CACHE)
        try {
          const list = await this.env.QUESTIONS.list({ prefix: CONSTANTS.LOWCARD_WINNER_KEY });
          for (const key of list.keys) {
            const roomName = key.name.replace(CONSTANTS.LOWCARD_WINNER_KEY, '');
            if (roomName) {
              const winners = await this.env.QUESTIONS.get(key.name, 'json');
              if (winners && typeof winners === 'object') {
                this._kvCache.set(`winners_${roomName}`, winners, 60000);
              }
            }
          }
        } catch(e) {}
      ...
      // START DICE JIKA WAKTUNYA
      setTimeout(() => {
        if (!this.closing && !this.isDestroyed && this._isDiceTime()) {
          this.diceAutoEnabled = true;
          setTimeout(() => {
            if (!this.closing && !this.isDestroyed && !this.currentDiceRoll) {
              this.forceStartDice();
            }
          }, 3000);
        }
  ...
  // GET RECORDING STATUS - DARI CACHE SAJA
  async _getRecordingStatusFromKV(roomName) {
    try {
      ...
      // JIKA TIDAK ADA DI CACHE, RETURN false
      return false;
    } catch(e) {
      return false;
    }
  ...
  // GET WINNERS - DARI CACHE SAJA
  async _getLowCardWinners(room) {
    try {
      if (!room) return {};
      
      // CEK RECORDING STATUS DARI CACHE
      const isRecording = this._recordingEnabled.get(room) || false;
      if (!isRecording) {
        return {};
      }
      ...
      // JIKA TIDAK ADA DI CACHE, RETURN {}
      return {};
    } catch(e) {
      return {};
    }
  ...
  // SET RECORDING STATUS - UPDATE KV DAN CACHE
    ...
    try {
      ...
      if (this.env?.QUESTIONS) {
        if (status) {
          ...
          await this.env.QUESTIONS.delete(
            CONSTANTS.LOWCARD_RECORDING_KEY + roomName
          );
        }
      }
  ...
  // SET WINNERS - UPDATE KV DAN CACHE
  async _setLowCardWinners(room, winners) {
    try {
      if (!room) return false;
      
      // UPDATE CACHE
      this._kvCache.set(`winners_${room}`, winners, 60000);
  ...
  // DELETE WINNERS - DELETE KV DAN CACHE
  ...
  // ADD WINNER - UPDATE KV DAN CACHE
  async _addLowCardWinner(room, username) {
    try {
      ...
      // AMBIL WINNERS DARI CACHE
      let winners = this._kvCache.get(`winners_${room}`) || {};
      
      let currentCount = 0;
      if (winners[username]) {
        const valStr = String(winners[username]);
        currentCount = parseInt(valStr.replace("x", "").replace("X", "")) || 0;
      }
      const newCount = currentCount + 1;
      
      winners[username] = newCount + "x";
      
      // UPDATE CACHE DAN KV
      await this._setLowCardWinners(room, winners);
  ...
  async _startRecordingWinners(roomName) {
    try {
      ...
      // CEK CACHE
      const currentStatus = this._recordingEnabled.get(roomName) || false;
      if (currentStatus) {
        this._broadcastToRoom(roomName, ["recordingStatus", true]);
        return true;
      }
      
      // SET STATUS (UPDATE KV + CACHE)
  ...
  async _stopRecordingWinners(roomName) {
    try {
      ...
      // CEK CACHE
      const currentStatus = this._recordingEnabled.get(room) || false;
      if (!currentStatus) {
      ...
      // SET STATUS (UPDATE KV + CACHE)
      const success = await this._setRecordingStatus(room, false);
      if (success) {
        // HAPUS WINNERS DARI CACHE DAN KV
  ...
  async _sendWinnersToRoom(room) {
    try {
      ...
      // AMBIL WINNERS DARI CACHE
      const winners = this._kvCache.get(`winners_${room}`) || {};
  ...
  _checkStuckGames() {
    try {
      ...
      for (const [room, game] of this.activeGames) {
        ...
        // CEK REGISTRATION STUCK
        if (game.registrationOpen && game._createdAt && 
            (now - game._createdAt) > CONSTANTS.STUCK_REGISTRATION_TIMEOUT_MS) {
  ...
  _cleanupStaleGames() {
    try {
      ...
      for (const [room, game] of this.activeGames) {
        ...
        // GAME STALE (TIDAK AKTIF DAN SUDAH LAMA)
        if ((game._gameEnded || !game._isActive) && game._endTime && 
            (now - game._endTime) > CONSTANTS.STALE_GAME_TIMEOUT_MS) {
          this._deleteGame(room, game);
          continue;
        }
        
        // GAME AKTIF TAPI TIDAK ADA PLAYER DAN SUDAH LAMA
        if (game._isActive && !game._gameEnded && game.players && 
            game.players.size === 0 && game._createdAt && 
            (now - game._createdAt) > CONSTANTS.STALE_GAME_TIMEOUT_MS) {
          this._deleteGame(room, game);
        }
      }
    } catch(e) {}
  ...
  _cleanupDeadConnections() {
    try {
      const now = Date.now();
      const staleThreshold = CONSTANTS.MAX_IDLE_TIME_MS || 300000;
      ...
      for (const [wsId, ws] of this.wsMap) {
        ...
        // CEK STALE CONNECTION (TERLALU LAMA TIDAK AKTIF)
        if (ws._lastActivity && (now - ws._lastActivity) > staleThreshold) {
          try {
            if (ws.readyState === 1) {
      ...
      if (url.pathname === "/health") {
        try {
          const status = {
            ...
            recordingRooms: Array.from(this._recordingEnabled.keys())
          };
