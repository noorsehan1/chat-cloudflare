// ==================== GAME-SERVER.JS ====================
// VERSION: 4.1.0 - CLEANUP ONLY ON ALARM (QUIZ SCHEDULE)
// WEBSOCKET HIBERNATION SUPPORT - NO INTERVAL
...
export class GameServer {
  ...
  async _restoreAllState() {
    try {
      // 1. Restore cache manager
      await this.cacheManager.restore(this.env);
      
      // 2. Restore dice game system
    ...
    try {
      ...
      for (const session of QUIZ_SCHEDULE.SESSIONS) {
        ...
        if (currentTotal < startTotal) {
          // Session di masa depan hari ini
  ...
  async alarm() {
    ...
    // Cek apakah sedang dalam jadwal quiz
    ...
    if (inSchedule) {
      // ===== DALAM JADWAL QUIZ =====
      // ✅ CLEANUP HANYA SAAT ALARM (DI JADWAL QUIZ)
  ...
  _initLazy() {
    ...
    try {
      ...
      // ✅ TIDAK ADA INTERVAL! Semua via alarm
