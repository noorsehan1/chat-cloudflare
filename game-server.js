...
const CONSTANTS = {
  ...
  BROADCAST_BUFFER_FLUSH_MS: 200,
  NOTIFICATION_THROTTLE_MS: 3000,
  LEADERBOARD_CACHE_TTL_MS: 10000,
};
...
export class GameServer extends CPUProtection {
  constructor(state, env) {
    try {
      ...
      this._masterInterval = setInterval(() => {
        this._masterTick();
      }, CONSTANTS.MASTER_INTERVAL_MS);

      this._bufferFlushTimer = setInterval(() => {
        this._flushBroadcastBuffer();
      }, CONSTANTS.BROADCAST_BUFFER_FLUSH_MS);
  ...
  _masterTick() {
    try {
      ...
      if (now - this._leaderboardCache.timestamp > CONSTANTS.LEADERBOARD_CACHE_TTL_MS) {
        this._refreshLeaderboardCache();
      }
  ...
  async _checkAndResetWeeklyDice() {
    try {
      ...
      const isResetTime = hours === 0 && minutes === 0 && seconds < 60;
      
      if (weekChanged) {
        const weekA = parseInt(lastResetWeek.split('-W')[1]);
        const weekB = parseInt(currentWeek.split('-W')[1]);
        const delayWeeks = weekB - weekA;
        
        let daysSinceMonday = dayOfWeek === 0 ? 6 : dayOfWeek - 1;
        const delayDays = daysSinceMonday;
        const delayHours = hours;
        const delayMinutes = minutes;
        
        const totalDelaySeconds = (delayWeeks * 7 * 24 * 60 * 60) + 
                                  (delayDays * 24 * 60 * 60) + 
                                  (delayHours * 60 * 60) + 
                                  (delayMinutes * 60) + 
                                  seconds;
        ...
        if (isMonday && isResetTime) {
          await this._performWeeklyReset(lastResetWeek, currentWeek);
          return true;
        } else if (weekChanged) {
          const resetKey = `weekly_reset_${currentWeek}`;
          const alreadyReset = await this.env.QUESTIONS.get(resetKey);
          ...
          await this._performWeeklyReset(lastResetWeek, currentWeek);
          await this.env.QUESTIONS.put(resetKey, 'true');
          return true;
        }
      }
  ...
  async _performWeeklyReset(lastResetWeek, currentWeek) {
    try {
      const points = await this.env.QUESTIONS.get(CONSTANTS.DICE_POINT_KEY, 'json') || {};
      ...
      this._broadcastToRoom(DICE_ROOM, ["weeklyReset", {
        oldWeek: lastResetWeek,
        newWeek: currentWeek,
        winner: winner,
        score: highestScore,
        timestamp: Date.now()
  ...
  _flushBroadcastBuffer() {
    try {
      if (this.closing || this.isDestroyed) return;
      if (!this._broadcastBuffer) return;
      
      for (const [room, messages] of this._broadcastBuffer) {
  ...
  // ==================== LEADERBOARD CACHE ====================
  async _refreshLeaderboardCache() {
    try {
      if (!this._leaderboardCache) {
        this._leaderboardCache = { data: null, timestamp: 0 };
      }
      if (!this.env?.QUESTIONS) return;
      ...
      this._leaderboardCache.data = sorted.map(([username, score]) => `${username}|${score}`);
      this._leaderboardCache.timestamp = Date.now();
  ...
  async submitDiceAnswer(ws, username, guess) {
    try {
      ...
      if (!this._playerAnswers) this._playerAnswers = new Map();
  ...
  async _startTieBreaker(room, players) {
    ...
    if (!this._tieBreakers) this._tieBreakers = new Map();
  ...
  _getActiveTieBreakerId() {
    if (!this._tieBreakers) return null;
  ...
  _broadcastDiceNotification(type, data) {
    try {
      ...
      if (message !== "TIME UP") {
        if (this._lastNotificationKey === key && (now - this._lastNotificationTime) < CONSTANTS.NOTIFICATION_THROTTLE_MS) {
          return;
        }
    ...
    try {
      ...
      for (const wsId of toRemove) {
        ...
        if (ws) {
          ...
          if (this._diceTimeLeftNotified) this._diceTimeLeftNotified.delete(wsId);
          if (this._nextDiceNotified) this._nextDiceNotified.delete(wsId);
          if (this._diceJoinedNotified) this._diceJoinedNotified.delete(wsId);
  ...
  _getWsId(ws) {
    try {
      ...
      const newId = ++this._wsIdCounter;
      ws._wsId = newId;
      this.wsMap.set(newId, ws);
      return newId;
    } catch(e) {
      return null;
    }
  ...
  async switchRoom(ws, room, username = null) {
    try {
      ...
      if (!this._switchLocks) this._switchLocks = new Map();
      if (!this._switchRetries) this._switchRetries = new Map();
      
      const roomName = room.trim();
      const wsId = this._getWsId(ws);
      
      if (!wsId) {
        const newWsId = ++this._wsIdCounter;
        ws._wsId = newWsId;
        this.wsMap.set(newWsId, ws);
        
        this._addClient(roomName, ws, username, true);
        this._safeSend(ws, ["switchRoomSuccess", roomName]);
        this._sendGameStateToClient(ws, roomName);
        
        if (roomName === DICE_ROOM) {
          this._sendDiceNotificationOnSwitch(ws, newWsId);
        }
        return;
      }
      ...
      if (currentRoom === roomName) {
        ...
        if (!this.wsClients.get(roomName).has(wsId)) {
      ...
      try {
        ...
        setTimeout(() => {
          if (this._switchLocks) this._switchLocks.delete(lockKey);
          if (this._switchRetries) this._switchRetries.delete(lockKey);
        }, 2000);
      }
    } catch(e) {
      if (this._switchLocks) this._switchLocks.delete(`switch_${this._getWsId(ws)}`);
  ...
  _sendGameStateToClient(ws, room) {
    try {
      ...
      let targetRoom = room || this._getRoomForWs(ws);
      if (!targetRoom) {
      ...
      this._getRecordingStatusFromKV(targetRoom).then(isRecording => {
  ...
  _sendDiceNotificationOnSwitch(ws, wsId) {
    try {
      ...
      if (isGameActive) {
        ...
        if (this._diceJoinedNotified) this._diceJoinedNotified.set(wsId, true);
      }
  ...
  _broadcastToRoom(room, message) {
    try {
      if (this.closing || this.isDestroyed || !room || !message) return;
      this._bufferBroadcast(room, message);
    } catch(e) {}
  }
  ...
  async _processEventQueue() {
    try {
      if (this._isProcessingQueue || !this._eventQueue || this._eventQueue.length === 0) return;
  ...
  async _handleEventInternal(ws, data) {
    try {
      ...
      if (evt === "switchRoom") {
        const room = data[1];
        if (!room) {
          this._safeSend(ws, ["gameLowCardError", "Room name required"]);
          return;
        }
        const username = data[2] || null;
      ...
      if (evt === "getDiceLeaderboard") {
        try {
          ...
          if (this._leaderboardCache.data && 
              (Date.now() - this._leaderboardCache.timestamp) < CONSTANTS.LEADERBOARD_CACHE_TTL_MS) {
            const result = this._leaderboardCache.data.slice(0, limit);
  ...
  async destroy() {
    try {
      ...
      if (this._bufferFlushTimer) {
        clearInterval(this._bufferFlushTimer);
        this._bufferFlushTimer = null;
      }
  ...
  async webSocketMessage(ws, msg) {
    try {
      ...
      if (data[0] === "switchRoom") {
        const room = data[1];
        if (!room) {
          this._safeSend(ws, ["gameLowCardError", "Room name required"]);
          return;
        }
        const username = data[2] || ws.username || null;
        await this.switchRoom(ws, room, username);
        return;
      }
