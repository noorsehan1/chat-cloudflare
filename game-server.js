// ==================== GAME-SERVER.JS ====================
// VERSION: 7.0.1 - WITH HYBERNATE API + ALARM
...
export class GameServer {
  ...
  constructor(state, env) {
    try {
      ...
      this.lowcardGames = new Map();      // active lowcard games
      this.lowcardTimers = new Map();     // game timers
      this.lowcardLocks = new Map();      // game locks
  ...
  async _restoreAlarm() {
    try {
      if (!this.ctx) return;
      const alarmData = await this.ctx.storage.get("alarmData");
      if (alarmData && alarmData.nextAlarm) {
        const remaining = alarmData.nextAlarm - Date.now();
        if (remaining > 0) {
          await this.ctx.storage.setAlarm(Date.now() + remaining);
        } else {
          await this.ctx.storage.setAlarm(Date.now() + CONSTANTS.ALARM_INTERVAL_MS);
        }
      } else {
        await this.ctx.storage.setAlarm(Date.now() + CONSTANTS.ALARM_INTERVAL_MS);
      }
  ...
  async handleAlarm() {
    try {
      ...
      // 6. SET ALARM BERIKUTNYA
      await this.ctx.storage.put("alarmData", { 
        nextAlarm: Date.now() + CONSTANTS.ALARM_INTERVAL_MS 
      });
      await this.ctx.storage.setAlarm(Date.now() + CONSTANTS.ALARM_INTERVAL_MS);
  ...
  _cleanupExpiredGames() {
    try {
      ...
      const toRemove = [];
      for (const [room, game] of this.lowcardGames) {
        if (game._gameEnded && (now - game._endTime) > CONSTANTS.GAME_CLEANUP_DELAY_MS) {
          toRemove.push(room);
        }
        if (game._isActive && !game._gameEnded && (now - game._createdAt) > 3600000) {
      ...
      for (const room of toRemove) {
        this.lowcardGames.delete(room);
        this.lowcardTimers.delete(room);
        this.lowcardLocks.delete(room);
  ...
  async startLowcardGame(ws, bet, username) {
    try {
      if (this.isDestroyed) {
      ...
      const existingGame = this.lowcardGames.get(room);
      if (existingGame?._isActive && !existingGame._gameEnded) {
        this._safeSend(ws, ["gameLowCardError", "Game is already running"]);
        return;
      }
      if (existingGame) await this._forceCleanupLowcardGame(room, existingGame);
      ...
      this.lowcardGames.set(room, game);
      ...
      this._startLowcardRegistration(room, game);
  ...
  async joinLowcardGame(ws, username) {
    try {
      if (this.isDestroyed) {
        this._safeSend(ws, ["gameLowCardError", "Server is shutting down"]);
        return;
  ...
  async submitLowcardNumber(ws, number, tanda, username) {
    try {
      if (this.isDestroyed) {
        this._safeSend(ws, ["gameLowCardError", "Server is shutting down"]);
        return;
      ...
      if (game.numbers.size === activeIds.length && !game.evaluationLocked && !game.drawTimeExpired &&
        ...
        const evalTimer = this._trackTimer(setTimeout(() => {
          try { this._evaluateLowcardRound(room, game); } catch(e) {}
  ...
  async leaveLowcardGame(ws, username) {
    try {
      if (this.isDestroyed) {
        this._safeSend(ws, ["gameLowCardError", "Server is shutting down"]);
        return;
      ...
      this._removePlayerFromLowcardGame(usernameClean, room);
    } catch(e) {}
  }

  async checkLowcardGameRunning(ws, roomname) {
    try {
      if (this.isDestroyed) {
  ...
  _removePlayerFromLowcardGame(username, room) {
    try {
      const game = this.lowcardGames.get(room);
      ...
      const checkTimer = setTimeout(() => {
        try {
          const currentGame = this.lowcardGames.get(room);
          if (currentGame && currentGame === game && !game._gameEnded) this._checkLowcardGameCanContinue(room, game);
        } catch(e) {}
      }, 1000);
  ...
  async _checkLowcardGameCanContinue(room, game) {
    try {
      ...
      if (activePlayers.length === 0) {
        ...
        game._isActive = false;
        this._broadcastToRoom(room, ["gameLowCardEnd", []]);
        this._forceCleanupLowcardGame(room, game);
        return;
      }
  ...
  async _forceCleanupLowcardGame(room, game) {
  ...
  _startLowcardRegistration(room, game) {
    try {
      ...
      setTimeout(() => {
        if (this._isGameActuallyRunning(game) && game.registrationOpen) {
          this._broadcastToRoom(room, ["gameLowCardTimeLeft", "10s"]);
        }
      }, 10000);
      
      setTimeout(() => {
        if (this._isGameActuallyRunning(game) && game.registrationOpen) {
          this._broadcastToRoom(room, ["gameLowCardTimeLeft", "5s"]);
        }
      }, 15000);
      ...
      const timer = this._trackTimer(setTimeout(() => {
        if (this._isGameActuallyRunning(game) && game.registrationOpen) {
          ...
          this._closeLowcardRegistration(room, game);
        }
      }, 20000));
  ...
  _closeLowcardRegistration(room, game) {
    try {
      if (!this._isGameActuallyRunning(game) || !game.registrationOpen) return;
  ...
  _scheduleLowcardCleanup(room, game) {
    try {
      if (!room || !game) return;
      if (this._cleanupTimers.has(room)) {
        this._clearTimer(this._cleanupTimers.get(room));
      ...
      const timer = this._trackTimer(setTimeout(() => {
        ...
        this._cleanupTimers.delete(room);
        const gameToDelete = this.lowcardGames.get(room);
        if (gameToDelete) this._forceCleanupLowcardGame(room, gameToDelete);
      }, CONSTANTS.GAME_CLEANUP_DELAY_MS));
      this._cleanupTimers.set(room, timer);
    } catch(e) {}
  }

  async _startLowcardDrawPhase(room, game) {
    const lockKey = `startDraw_${room}`;
    if (this.lowcardLocks.has(lockKey)) return;
    if (!this._acquireLock(this.lowcardLocks, lockKey, 10000)) return;
    try {
      if (!this._isGameActuallyRunning(game)) {
        this._releaseLock(this.lowcardLocks, lockKey);
        return;
      }
      ...
      this._releaseLock(this.lowcardLocks, lockKey);
      this._startLowcardDrawCountdown(room, game);
      if (game.botPlayers?.size > 0 && this._isGameActuallyRunning(game)) {
        this._startLowcardBotDraws(room, game);
      }
    } catch(e) {
      this._releaseLock(this.lowcardLocks, lockKey);
    }
  ...
  _startLowcardDrawCountdown(room, game) {
    try {
      if (!this._isGameActuallyRunning(game)) return;
      if (game._drawTimer) {
      ...
      const timer = this._trackTimer(setTimeout(() => {
        if (this._isGameActuallyRunning(game) && !game.drawTimeExpired) {
          this._broadcastToRoom(room, ["gameLowCardTimeLeft", "TIME UP"]);
          this._closeLowcardDrawPhase(room, game);
        }
      }, 20000));
  ...
  _startLowcardBotDraws(room, game) {
    try {
      ...
      for (const botId of notDrawn) {
        ...
        const timeout = this._trackTimer(setTimeout(() => {
          ...
          if (this._isGameActuallyRunning(currentGame) && !currentGame.drawTimeExpired &&
            ...
            this._handleLowcardBotDraw(room, botId, currentGame);
          }
  ...
  _handleLowcardBotDraw(room, botId, game) {
    try {
  ...
  _forceLowcardBotDraw(room, botId, game) {
    try {
  ...
  async _closeLowcardDrawPhase(room, game) {
    const drawLockKey = `draw_${room}`;
    if (this.lowcardLocks.has(drawLockKey)) return;
    if (!this._acquireLock(this.lowcardLocks, drawLockKey, 10000)) return;
    try {
      if (!this._isGameActuallyRunning(game) || game.drawTimeExpired || game.evaluationLocked) {
        this._releaseLock(this.lowcardLocks, drawLockKey);
        return;
      }
      ...
      if (game.botPlayers?.size > 0 && this._isGameActuallyRunning(game)) {
        ...
        for (const botId of activeBotIds) {
          this._forceLowcardBotDraw(room, botId, game);
        }
      }
  ...
  async _evaluateLowcardRound(room, game) {
    const evalLockKey = `eval_${room}`;
    if (this.lowcardLocks.has(evalLockKey)) return;
    if (!this._acquireLock(this.lowcardLocks, evalLockKey, 15000)) return;
    try {
      if (this.isDestroyed || !game?._isActive || game._gameEnded || game._isEvaluating || !game.players) {
        this._releaseLock(this.lowcardLocks, evalLockKey);
        return;
      }
      ...
      const safetyTimer = this._trackTimer(setTimeout(() => {
        if (game?._isEvaluating) { game._isEvaluating = false; this._scheduleLowcardCleanup(room, game); }
        this._releaseLock(this.lowcardLocks, evalLockKey);
      ...
      this._releaseLock(this.lowcardLocks, `eval_${room}`);
    }
  }
