...
export class GameServer {
  ...
  _loadKVData() {
    try {
      ...
      this.env.QUESTIONS.get(CONSTANTS.DICE_LAST_RESET_WEEK).then((existing) => {
        if (!existing) {
  ...
  _getLastWeekWinnerAndReset() {
    try {
      if (!this.env?.QUESTIONS) return null;
      const currentWeek = this._generateCurrentWeek(new Date());
      const lastResetWeek = this._cachedResetWeek;
  ...
  _performReset() {
    try {
      ...
      this.diceGameSystem.resetPoints().catch(() => {});
  ...
  _handleBotDraw(room, botId, game) {
    try {
      ...
      const number = game.round <= 2 ? Math.floor(Math.random() * 12) + 1 : 
        Math.random() < 0.6 ? [8, 9, 10, 11, 12][Math.floor(Math.random() * 5)] : 
        [1, 2, 3, 4, 5, 6, 7][Math.floor(Math.random() * 7)];
      const tanda = ["C1", "C2", "C3", "C4"][Math.floor(Math.random() * 4)];
