...
export class GameWorker {
    ...
    async _loadInitialData() {
        try {
            for (const room of ROOMS) {
                ...
                this._recordingEnabled.set(room, status);
            }
        } catch(e) {
    ...
    async _getRecordingStatus(roomName) {
        try {
            const result = await this.env.DB.prepare(
                `SELECT is_recording FROM recording_status WHERE room_name = ?`
            ).bind(roomName).first();
            return result?.is_recording === 1;
        } catch(e) {
            return false;
        }
    }

    async _setRecordingStatus(roomName, status) {
        try {
            await this.env.DB.prepare(
                `INSERT OR REPLACE INTO recording_status (room_name, is_recording, updated_at) VALUES (?, ?, ?)`
            ).bind(roomName, status ? 1 : 0, Math.floor(Date.now() / 1000)).run();
            this._recordingEnabled.set(roomName, status);
            return true;
        } catch(e) {
            return false;
        }
    ...
    async _getLowCardWinners(roomName) {
        try {
            const result = await this.env.DB.prepare(
                `SELECT username, wins FROM lowcard_winners WHERE room_name = ?`
            ).bind(roomName).all();

            const winners = {};
            for (const row of result.results || []) {
                winners[row.username] = row.wins + "x";
            }
            return winners;
        } catch(e) {
            return {};
    ...
    async _addLowCardWinner(roomName, username) {
        try {
            const isRecording = await this._getRecordingStatus(roomName);
            if (!isRecording) return false;

            const result = await this.env.DB.prepare(
                `SELECT wins FROM lowcard_winners WHERE room_name = ? AND username = ?`
            ).bind(roomName, username).first();

            const currentWins = result?.wins || 0;
            await this.env.DB.prepare(
                `INSERT OR REPLACE INTO lowcard_winners (room_name, username, wins, updated_at) VALUES (?, ?, ?, ?)`
            ).bind(roomName, username, currentWins + 1, Math.floor(Date.now() / 1000)).run();
    ...
    async _clearLowCardWinners(roomName) {
        try {
            await this.env.DB.prepare(
                `DELETE FROM lowcard_winners WHERE room_name = ?`
            ).bind(roomName).run();
            return true;
        } catch(e) {
            return false;
    ...
    async _getDicePoints() {
        try {
            const result = await this.env.DB.prepare(
                `SELECT username, points FROM dice_points ORDER BY points DESC`
            ).all();
            const points = {};
            for (const row of result.results || []) {
                points[row.username] = row.points || 0;
            }
            return points;
        } catch(e) {
            return {};
    ...
    async _addDicePoint(username) {
        try {
            const result = await this.env.DB.prepare(
                `SELECT points FROM dice_points WHERE username = ?`
            ).bind(username).first();

            const currentPoints = result?.points || 0;
            await this.env.DB.prepare(
                `INSERT OR REPLACE INTO dice_points (username, points, updated_at) VALUES (?, ?, ?)`
            ).bind(username, currentPoints + 1, Math.floor(Date.now() / 1000)).run();
    ...
    async _getLastWeekWinner() {
        try {
            const result = await this.env.DB.prepare(
                `SELECT * FROM dice_last_week_winner WHERE id = 1`
            ).first();
            return result;
        } catch(e) {
            return null;
    ...
    async _setLastWeekWinner(username, score, week) {
        try {
            await this.env.DB.prepare(
                `INSERT OR REPLACE INTO dice_last_week_winner (id, username, score, week, timestamp) VALUES (1, ?, ?, ?, ?)`
            ).bind(username, score || 0, week, Math.floor(Date.now() / 1000)).run();
            return true;
        } catch(e) {
            return false;
        }
    ...
    async _deleteLastWeekWinner() {
        try {
            await this.env.DB.prepare(`DELETE FROM dice_last_week_winner WHERE id = 1`).run();
            return true;
        } catch(e) {
            return false;
        }
    ...
    async _getLastResetWeek() {
        try {
            const result = await this.env.DB.prepare(
                `SELECT week FROM dice_last_reset_week WHERE id = 1`
            ).first();
            return result?.week || null;
        } catch(e) {
            return null;
        }
    }
    ...
    async _setLastResetWeek(week) {
        try {
            await this.env.DB.prepare(
                `INSERT OR REPLACE INTO dice_last_reset_week (id, week, updated_at) VALUES (1, ?, ?)`
            ).bind(week, Math.floor(Date.now() / 1000)).run();
            return true;
        } catch(e) {
            return false;
        }
    ...
    async _getActiveGame(roomName) {
        try {
            const result = await this.env.DB.prepare(
                `SELECT * FROM active_games WHERE room_name = ? AND is_active = 1 AND game_ended = 0`
            ).bind(roomName).first();

            if (!result) return null;

            const players = await this.env.DB.prepare(
                `SELECT username, is_bot, eliminated FROM game_players WHERE room_name = ?`
            ).bind(roomName).all();

            const numbers = await this.env.DB.prepare(
                `SELECT username, number_value, tanda FROM game_numbers WHERE room_name = ?`
            ).bind(roomName).all();
            ...
            const game = {
                ...
                players: new Map(),
                botPlayers: new Map(),
                registrationOpen: result.registration_open === 1,
                round: result.round || 1,
                numbers: new Map(),
                tanda: new Map(),
                eliminated: new Set(),
                betAmount: result.bet_amount || 0,
                hostName: result.host_name || '',
                useBots: false,
                evaluationLocked: false,
                drawTimeExpired: false,
                _isActive: result.is_active === 1,
                _gameEnded: result.game_ended === 1,
                _phase: result.phase || 'registration',
                ...
                _safetyTimer: null,
                _isEvaluating: false,
                _createdAt: result.created_at || Date.now(),
                _drawPhaseStart: null,
                _endTime: null,
                playerWsId: new Map(),
                _startedByRecording: result.started_by_recording === 1,
                _startedBy: result.started_by_recording === 1 ? 'recording' : 'user'
            };

            for (const p of players.results || []) {
                game.players.set(p.username, { id: p.username, name: p.username });
                if (p.is_bot === 1) {
                    game.botPlayers.set(p.username, p.username);
                }
                if (p.eliminated === 1) {
                    game.eliminated.add(p.username);
                }
            }

            for (const n of numbers.results || []) {
                game.numbers.set(n.username, n.number_value);
                if (n.tanda) {
                    game.tanda.set(n.username, n.tanda);
                }
            }
    ...
    async _saveGameState(roomName, game) {
        try {
            await this.env.DB.prepare(`
                INSERT OR REPLACE INTO active_games 
                (room_name, bet_amount, host_name, round, phase, registration_open, is_active, game_ended, started_by_recording, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `).bind(
                roomName,
                game.betAmount || 0,
                game.hostName || '',
                game.round || 1,
                game._phase || 'registration',
                game.registrationOpen ? 1 : 0,
                game._isActive ? 1 : 0,
                game._gameEnded ? 1 : 0,
                game._startedByRecording ? 1 : 0,
                Math.floor(Date.now() / 1000)
            ).run();

            await this.env.DB.prepare(
                `DELETE FROM game_players WHERE room_name = ?`
            ).bind(roomName).run();

            for (const [username] of game.players) {
                const isBot = game.botPlayers.has(username) ? 1 : 0;
                const eliminated = game.eliminated.has(username) ? 1 : 0;
                await this.env.DB.prepare(
                    `INSERT INTO game_players (room_name, username, is_bot, eliminated) VALUES (?, ?, ?, ?)`
                ).bind(roomName, username, isBot, eliminated).run();
            }

            await this.env.DB.prepare(
                `DELETE FROM game_numbers WHERE room_name = ?`
            ).bind(roomName).run();

            for (const [username, value] of game.numbers) {
                const tanda = game.tanda.get(username) || '';
                await this.env.DB.prepare(
                    `INSERT INTO game_numbers (room_name, username, number_value, tanda) VALUES (?, ?, ?, ?)`
                ).bind(roomName, username, value, tanda).run();
            }
    ...
    async _deleteGameState(roomName) {
        try {
            await this.env.DB.prepare(
                `DELETE FROM active_games WHERE room_name = ?`
            ).bind(roomName).run();
            await this.env.DB.prepare(
                `DELETE FROM game_players WHERE room_name = ?`
            ).bind(roomName).run();
    ...
    async _endDiceRound() {
        try {
            ...
            if (correctPlayers.length === 0) {
                ...
                const newPoints = await this._addDicePoint(winner);

                this._broadcastToRoom(DICE_ROOM, ["diceWinner", {
                    username: winner,
                    totalPoints: newPoints || 0,
                    diceValue: diceValue,
                    round: roundNumber
                }]);
    ...
    async _getLastWeekWinnerAndReset() {
        try {
            ...
            if (!lastResetWeek || weekChanged) {
                ...
                return await this._getLastWeekWinner();
            }
    ...
    async _performReset() {
        try {
            ...
            if (winner && highestScore > 0) {
                await this._setLastWeekWinner(winner, highestScore, lastResetWeek || currentWeek);
            } else {
                await this._deleteLastWeekWinner();
            }

            await this.env.DB.prepare(`DELETE FROM dice_points`).run();
            ...
            return { winner, score: highestScore, week: currentWeek };
        } catch(e) {
            return null;
        }
    }
    ...
    async _handleEventInternal(ws, data) {
        try {
            ...
            if (evt === "deleteDiceLastWeekWinner") {
                try {
                    const success = await this._deleteLastWeekWinner();
            ...
            if (evt === "stopRecordingWinners") {
                ...
                if (success) {
                    await this._clearLowCardWinners(roomName);
    ...
    _scheduleGameCleanup(room, game) {
        try {
            ...
            const timer = this._trackTimer(setTimeout(async () => {
                ...
                if (gameToDelete) {
                    await this._deleteGame(room, gameToDelete);
                }
    ...
    async _deleteGame(room, game) {
        try {
            ...
            await this._deleteGameState(room);
    ...
    async startGame(ws, bet, username) {
        try {
            ...
            try {
                ...
                await this._saveGameState(room, game);
    ...
    async joinGame(ws, username) {
        try {
            ...
            try {
                ...
                if (!game) {
                    const dbGame = await this._getActiveGame(room);
                    if (dbGame && dbGame._isActive && !dbGame._gameEnded) {
                        this.activeGames.set(room, dbGame);
                        game = dbGame;
                    }
                }
        ...
        try {
            ...
            if (url.pathname === "/game/ws") {
                ...
                try {
                    ...
                    try {
                        server.close(1008, "Accept failed");
                    } catch(err) {}
            ...
            return new Response("Game Server - D1 Version", { status: 200 });
