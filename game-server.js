// ==================== GAME-SERVER.JS ====================
// VERSION: 3.1.1 - OPTIMIZED FREE TIER - FULL CLASS

const CONSTANTS = {
  MAX_LOWCARD_GAMES: 5,
  REGISTRATION_TIME_MS: 15000,
  DRAW_TIME_MS: 15000,
  EVALUATION_DELAY_MS: 1500,
  MAX_BOTS_PER_GAME: 3,
  MAX_BET: 100000,
  MAX_BOT_DRAWS_PER_ROUND: 3,
  ...
  DICE_AUTO_START_DELAY_MS: 2000,
  TIE_BREAKER_TIME_LIMIT: 15,
  TIE_BREAKER_COOLDOWN: 10000,
...
class KVCache {
  ...
  get(key) {
    ...
    if (Date.now() - entry.timestamp > CONSTANTS.CACHE_TTL_MS * 2) {
  ...
  set(key, value) {
    if (this.cache.size >= this._maxSize) {
      ...
      for (let i = 0; i < keys.length && this.cache.size >= this._maxSize; i++) {
...
class CacheManager {
  constructor() {
    ...
    this.cacheTTL = CONSTANTS.CACHE_TTL_MS || 30000;
    this._updateLocks = new Map();
    this._kvCache = new KVCache();
    this._pendingWrites = new Map();
    this._writeBatchTimer = null;
  }

  getRecordingStatus(room) {
    if (!room) return false;
    if (this.recordingStatus.has(room)) {
      return this.recordingStatus.get(room);
    }
    return false;
  }
  ...
  async addWinner(room, username, env) {
    ...
    try {
      ...
      this._addPendingWrite(room, roomWinners, env);
      return true;
    } catch(e) {
  ...
  _addPendingWrite(room, winners, env) {
    const key = CONSTANTS.LOWCARD_WINNER_KEY + room;
    this._pendingWrites.set(key, { room, winners, env, timestamp: Date.now() });
    
    if (this._writeBatchTimer) {
      clearTimeout(this._writeBatchTimer);
    }
    this._writeBatchTimer = setTimeout(() => {
  ...
  async _flushPendingWrites() {
    ...
    const writes = Array.from(this._pendingWrites.values());
    this._pendingWrites.clear();
    for (const write of writes) {
      try {
        const key = CONSTANTS.LOWCARD_WINNER_KEY + write.room;
        await write.env.QUESTIONS.put(key, JSON.stringify(write.winners));
      } catch(e) {}
    }
  }
  ...
  async _reloadWinnersFromKV(room, env) {
    try {
      ...
      if (cached) {
        this.winnersCache.set(room, { winners: cached, timestamp: Date.now() });
        return;
      }
      const winners = await env.QUESTIONS.get(key, 'json') || {};
      this._kvCache.set(key, winners);
      this.winnersCache.set(room, { winners: winners, timestamp: Date.now() });
    } catch(e) {}
  }
...
class DicePointsCache {
  ...
  async setPoints(points, env) {
    ...
    try {
      ...
      this._addPendingWrite(points, env);
      return true;
    } catch(e) {
  ...
  async addPointToUser(username, env) {
    ...
    try {
      ...
      if (!points) points = await this._getFromKV(env);
  ...
  _addPendingWrite(points, env) {
    const key = CONSTANTS.DICE_POINT_KEY;
    this._pendingWrites.set(key, { points, env, timestamp: Date.now() });
    if (this._writeBatchTimer) clearTimeout(this._writeBatchTimer);
    this._writeBatchTimer = setTimeout(() => this._flushPendingWrites(), 5000);
  }
  ...
  async _flushPendingWrites() {
    ...
    for (const write of writes) {
      try {
        await write.env.QUESTIONS.put(CONSTANTS.DICE_POINT_KEY, JSON.stringify(write.points));
      } catch(e) {}
    }
  }

  async _getFromKV(env) {
    try {
      if (!env?.QUESTIONS) return {};
      const cached = this._kvCache.get(CONSTANTS.DICE_POINT_KEY);
      if (cached) return cached;
      const points = await env.QUESTIONS.get(CONSTANTS.DICE_POINT_KEY, 'json') || {};
      this._kvCache.set(CONSTANTS.DICE_POINT_KEY, points);
      return points;
    } catch(e) { return {}; }
  }
  ...
  getLeaderboard(limit = 10) {
    ...
    if (cached && Date.now() - cached.timestamp < this.leaderboardTTL) {
  ...
  async resetPoints(env) {
    ...
    try {
      this.pointsCache.delete('points');
      this.leaderboardCache.delete('leaderboard');
      this._kvCache.delete(CONSTANTS.DICE_POINT_KEY);
  ...
  cleanup() {
    ...
    for (const [key, data] of this.pointsCache) {
      if (now - data.timestamp > this.cacheTTL * 2) this.pointsCache.delete(key);
    }
    for (const [key, data] of this.leaderboardCache) {
      if (now - data.timestamp > this.leaderboardTTL * 2) this.leaderboardCache.delete(key);
    }
  }
...
class DiceGameSystem {
  ...
  cleanup() {
    if (this.pointsCache) this.pointsCache.cleanup();
  }
  
  clearCache() {
    this.userScores.clear();
    this._isLoaded = false;
    if (this.pointsCache) this.pointsCache.clear();
  }
}
...
export class GameServer {
  constructor(state, env) {
    try {
      ...
      this._diceNotifiedFlags = { 15: false, 10: false, 5: false, timeup: false };
  ...
  // ========== UTILITY LOCKS ==========
  _acquireLock(lockMap, key, timeoutMs = 3000) {
  ...
  _isLocked(lockMap, key) { return lockMap.has(key); }

  _cleanupStaleLocks() {
    const now = Date.now();
    const staleTimeout = 15000;
  ...
  _startDiceFast() {
    try {
      ...
      this._broadcastToRoom(DICE_ROOM, ["diceRoll", { 
        value, timestamp: Date.now(), answerTime: 15,
        canAnswerNow: true, round: this._diceRound
      }]);
      this._broadcastToRoom(DICE_ROOM, ["diceNotification", "♡ clik draw ♡"]);
      
      let timeLeft = 15;
      let notifSent = { 10: false, 5: false };
      
      const timerInterval = setInterval(() => {
        timeLeft--;
        if (timeLeft === 10 && !notifSent[10]) {
          notifSent[10] = true;
        ...
        try {
          ...
          if (success) {
            ...
            this._broadcastToRoom(DICE_ROOM, ["diceWinner", {
              ...
              diceValue: diceValue, round: roundNumber
            }]);
          } else {
            this._broadcastToRoom(DICE_ROOM, ["diceWinner", {
  ...
  async _startTieBreaker(room, players) {
    ...
    try {
      ...
      setTimeout(() => { this._tieLock = false; }, 1500);
    }
  }
  ...
  _startTieTimer(room, id, players) {
    ...
    let timeLeft = CONSTANTS.TIE_BREAKER_TIME_LIMIT || 15;
    ...
    }, (CONSTANTS.TIE_BREAKER_TIME_LIMIT || 15) * 1000 + 2000));
  }
  ...
  _startCooldownAfterTieBreaker() {
    this._broadcastToRoom(DICE_ROOM, ["diceNotification", "wait 10s"]);
    this._diceTimeUpCooldown = true;
    ...
    this._diceTimeUpCooldownTimer = this._trackTimer(setTimeout(() => {
      ...
      this._lastSentRemaining = -1;
      this._checkDice();
    }, CONSTANTS.TIE_BREAKER_COOLDOWN || 10000));
  }
  ...
  async _performReset() {
    try {
      ...
      if (winner && highestScore > 0) {
        const winnerData = { username: winner, score: highestScore, week: lastResetWeek || currentWeek, timestamp: Date.now() };
        await this._withTimeout(this.env.QUESTIONS.put(CONSTANTS.DICE_LAST_WEEK_WINNER, JSON.stringify(winnerData)), 1500);
  ...
  _getRandomDrawDelay() { return (Math.floor(Math.random() * 10) + 2) * 1000; }
  ...
  _fireAndForget(promise) { promise.catch(() => {}); }
  ...
  _handleBotDraw(room, botId, game) {
    try {
      if (!this._isGameActuallyRunning(game) || game.numbers?.has(botId) || 
          game.drawTimeExpired || game.evaluationLocked) return;
      ...
      if (game.numbers.size === activeIds.length && !game.evaluationLocked && 
          !game.drawTimeExpired && this._isGameActuallyRunning(game)) {
    ...
    try {
      ...
      if (!game._botsAdded) {
        if (humanCount === 1 || humanCount === 0) {
          ...
          const needed = Math.min(3 - game.players.size, CONSTANTS.MAX_BOTS_PER_GAME);
  ...
  async _startDrawPhase(room, game) {
    ...
    try {
      ...
      if (activePlayers.length < 2) {
        if (!game._botsAdded) {
          const needed = Math.min(3 - activePlayers.length, CONSTANTS.MAX_BOTS_PER_GAME);
          if (needed > 0) { this._addBots(room, needed); game._botsAdded = true; }
        }
  ...
  async _evaluateRound(room, game) {
    ...
    try {
      ...
      const currentGame = this.activeGames.get(room);
      if (currentGame !== game) { this._releaseLock(this._evaluationLocks, evalLockKey); return; }
      if (game._isEvaluating) { this._releaseLock(this._evaluationLocks, evalLockKey); return; }
  ...
  async _getRecordingStatusFromKV(roomName) {
    return this.cacheManager.getRecordingStatus(roomName);
  }
  ...
  async _broadcastLowCardWinners(room) {
    try {
      ...
      this._broadcastToRoom(room, ["lowCardWinnerUpdate", { winners: winners || {}, room, recording: true }]);
    } catch(e) {}
  }
  ...
  async startGame(ws, bet, username) {
    try {
      ...
      try {
        ...
        setTimeout(() => { this._gameLocks.delete(lockKey); }, 2000);
      }
    } catch(e) {}
  }
  ...
  async joinGame(ws, username) {
    try {
      ...
      try {
        ...
        setTimeout(() => { this._joinLocks.delete(lockKey); }, 1500);
      }
    } catch(e) {}
  }
  ...
  _removePlayerFromGame(username, room) {
    try {
      const game = this.activeGames.get(room);
      if (!game || !game.players?.has(username) || !game._isActive || game._gameEnded || 
          game._isEvaluating || game.evaluationLocked) return false;
  ...
  async _checkGameCanContinue(room, game) {
    try {
      if (!game?._isActive || game._gameEnded || !game.players || 
          game._isEvaluating || game.evaluationLocked || game.registrationOpen) return;
  ...
  async _processEventQueue(iteration = 0) {
    ...
    } catch(e) { this._handleError('processQueue', e); } finally {
      this._isProcessingQueue = false;
    }
  }
  ...
  async _handleEventInternal(ws, data) {
    try {
      ...
      if (evt === "getDiceLeaderboard") {
        try {
          let limit = data.length > 1 && typeof data[1] === 'number' ? Math.min(data[1], 20) : 10;
  ...
  async _processWithTimeout(ws, data, timeoutMs = 300) {
    try {
  ...
  _cleanupMemory() {
    try {
      if (this.wsMap) {
        ...
        for (const id of toRemove) { this.wsMap.delete(id); }
      }
      
      if (this._eventQueue && this._eventQueue.length > 30) {
        this._eventQueue.splice(0, this._eventQueue.length - 30);
      }
      
      const now = Date.now();
      let cleanedLocks = 0;
      for (const [key, time] of this._gameLocks) {
        if (cleanedLocks > 5) break;
        if (now - time > 30000) { this._gameLocks.delete(key); cleanedLocks++; }
      }
      for (const [key, time] of this._joinLocks) {
        if (cleanedLocks > 5) break;
        if (now - time > 30000) { this._joinLocks.delete(key); cleanedLocks++; }
      }
    } catch(e) {}
  }
  ...
  _broadcastToRoom(room, message) {
    try {
      ...
      const batchSize = CONSTANTS.BROADCAST_BATCH_SIZE || 5;
  ...
  async fetch(req) {
    try {
      if (this._circuitOpen) {
        const now = Date.now();
        if (now - this._lastResetTime > 30000) {
          this._circuitOpen = false;
          this._requestCount = 0;
          this._lastResetTime = now;
        } else {
      ...
      setTimeout(() => {
        this._requestCount = Math.max(0, this._requestCount - 25);
      ...
      if (url.pathname === "/game/ws") {
        ...
        server.addEventListener("close", () => { this.webSocketClose(server); }, { once: true });
        server.addEventListener("error", () => { this.webSocketError(server); }, { once: true });
