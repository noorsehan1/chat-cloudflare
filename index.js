// ChatServer Durable Object - FULL OVERWRITE VERSION (final)
...
export class ChatServer {
  constructor(state, env) {
    ...
    // Buffers (overwrite mode)
    // - updateKursiBuffer: Map(room -> Map(seat -> info))
    // - chatMessageBuffer: Map(room -> Array(lastMessage))  <-- keep as array for compatibility with flush code but we store only last
    // - privateMessageBuffer: Map(id -> Array(lastMessage))
  ...
  // flush chat buffer: each room only keeps last message (array length 1)
  flushChatBuffer() {
  ...
  // flush kursi updates: updateKursiBuffer holds last info per seat per room
  flushKursiUpdates() {
      ...
      switch (evt) {
        ...
        case "setIdTarget": {
          ...
          if (this.privateMessageBuffer.has(newId)) {
            // privateMessageBuffer stores arrays (but length 1 in overwrite mode)
        ...
        case "sendnotif": {
          ...
          if (!delivered) {
            // overwrite pending notif for idtarget
            this.privateMessageBuffer.set(idtarget, [notif]);
          }
          break;
        }
        ...
        case "private": {
          ...
          if (!delivered) {
            // overwrite previous private message for idt
        ...
        case "updateKursi": {
          ...
          if (!this.updateKursiBuffer.has(room))
            this.updateKursiBuffer.set(room, new Map());
          // overwrite buffer for that seat
        ...
        case "gift": {
          ...
          // overwrite last gift message for room
  ...
  async fetch(request) {
    try {
      ...
      // close: immediate cleanup (no schedule here)
      ...
      // error: schedule cleanup (grace period) and remove this socket ref
      ws.addEventListener("error", (error) => {
        const id = ws.idtarget;
        if (id && !this.cleanupInProgress.has(id)) {
          this.scheduleCleanupTimeout(id); // allow reconnect grace period
        }
