...
// Constants - OPTIMIZED FOR FREE TIER
const CONSTANTS = {
  LOCK_TIMEOUT: 10000,
  LOCK_ACQUIRE_TIMEOUT: 2000,
  GRACE_PERIOD: 5000,
  MAX_QUEUE_SIZE: 50,        // Turun dari 200
  MAX_LOCK_QUEUE_SIZE: 30,   // Turun dari 100
  MAX_POINTS_PER_ROOM: 50,   // Turun dari 200
  MAX_POINTS_TOTAL: 1000,    // Turun dari 5000
  MAX_POINTS_BEFORE_FLUSH: 200, // Turun dari 1000
  BUFFER_SIZE_LIMIT: 20,     // Turun dari 50
  CACHE_VALID_DURATION: 2000,
  MAX_SEATS: 35,             // Turun dari 35
  MAX_CONNECTIONS_PER_USER: 3, // Turun dari 5
  SAFE_SEND_RETRY: 2,
  SAFE_SEND_RETRY_DELAY: 100,
  BROADCAST_BATCH_SIZE: 10,  // Turun dari 20
...
class QueueManager {
  constructor(concurrency = 3) { // Turun dari 5
    this.queue = [];
    this.active = 0;
...
export class ChatServer {
  constructor(state, env) {
    try {
      ...
      this._pointFlushDelay = 100; // Lebih lambat dari 16ms
      ...
      // HANYA 1 TIMER untuk number tick
  ...
  async handleMessage(ws, raw) {
    ...
    try {
      ...
      try {
        ...
        // FLUSH setelah event (event-based)
...
export default {
  async fetch(req, env) {
    try {
      ...
      // Endpoint untuk cleanup manual (opsional)
  ...
  // ✅ TAMBAHKAN INI untuk cron trigger cleanup
